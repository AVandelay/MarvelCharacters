...
public func emptyAction<each Input, Output>(
    file: StaticString = #fileID,
    line: UInt = #line
...
public func emptyAction<each Input, Output>(
    returning defaultValue: @autoclosure @escaping () -> Output,
    file: StaticString = #fileID,
    line: UInt = #line
) -> (repeat each Input) -> Output {
    { (_: repeat each Input) -> Output in
...
public func emptyAction<each Input, Output>(
    throwing error: @autoclosure @escaping () -> any Error,
    file: StaticString = #fileID,
    line: UInt = #line
